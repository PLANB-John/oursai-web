...
export default function JoinGroup() {
  ...
  const isIljuValid = formData.userName.trim().length > 0 && formData.gender !== '' && formData.birthDate.length === 8;
    ...
    <div className="min-h-screen bg-[#f8f9fa] flex justify-center items-start sm:py-10 font-sans text-slate-800">
      ...
      <div className="w-full max-w-[480px] min-h-screen bg-white shadow-2xl flex flex-col relative overflow-hidden sm:rounded-[40px] pb-20">
        ...
        <main className="px-6 py-12 space-y-10">
          <h1 className="text-[28px] font-black text-slate-800 tracking-tight text-center">모임 참여하기</h1>
          <section className="space-y-8 px-2">
            ...
            {/* 태어난 시간 필드 추가 [cite: 2026-02-17] */}
            <div className="space-y-2">
              <label className="text-[14px] font-black text-slate-700">태어난 시간 (선택)</label>
              <input type="text" placeholder="1430" className="w-full p-4 bg-slate-50 rounded-2xl text-[14px] focus:outline-none" onChange={(e) => setFormData({...formData, birthTime: e.target.value})} />
              <p className="text-[11px] text-slate-300 font-medium ml-1">4자리 숫자로 입력해 주세요 (24시간 형식)</p>
            </div>

            <button onClick={handleJoin} disabled={!isIljuValid} className={`w-full py-6 rounded-[24px] font-black text-[18px] transition-all shadow-xl ${isIljuValid ? 'bg-[#6c5ce7] text-white shadow-purple-100 active:scale-95' : 'bg-slate-100 text-slate-300 cursor-not-allowed'}`}>
              👤+ 참여하기
            </button>
