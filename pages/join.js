...
import { supabase } from '../lib/supabaseClient'; // [1] 서버 연결 열쇠 가져오기 [cite: 2026-02-17]
...
export default function JoinGroup() {
  ...
  // [2] 서버 연동 참여하기 로직 [cite: 2026-02-17]
  const handleJoin = async () => {
    ...
    if (isIljuValid) {
      ...
      try {
        // 1. 해당 ID의 기존 방 데이터(멤버 리스트) 서버에서 불러오기 [cite: 2026-02-17]
        ...
        // 3. 기존 리스트에 나를 추가하여 서버 업데이트 (UPDATE 연동) [cite: 2026-02-17]
        ...
        // 4. 성공 시 해당 모임의 결과 페이지로 이동 [cite: 2026-02-17]
        router.push(`/g/${id}`);

      } catch (err) {
        alert("참여 처리 중 오류가 발생했습니다: " + err.message);
        setIsLoading(false);
      }
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] flex justify-center items-start sm:py-10 font-sans text-slate-800">
      ...
      <div className="w-full max-w-[480px] min-h-screen bg-white shadow-2xl flex flex-col relative overflow-hidden sm:rounded-[40px] pb-20">
        ...
        {/* 참여 처리 중 로딩 애니메이션 [cite: 2026-02-17] */}
        <AnimatePresence>
          {isLoading && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-[100] bg-white/95 backdrop-blur-sm flex flex-col items-center justify-center space-y-6">
              ...
              <p className="text-[18px] font-black text-slate-700">참여를 확정하는 중...</p>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <main className="px-6 py-12 space-y-10">
          ...
          <section className="space-y-8 px-2">
            ...
            <button 
              onClick={handleJoin} 
              disabled={!isIljuValid || isLoading} 
              className={`w-full py-6 rounded-[24px] font-black text-[18px] transition-all shadow-xl ${isIljuValid && !isLoading ? 'bg-[#6c5ce7] text-white shadow-purple-100 active:scale-95' : 'bg-slate-100 text-slate-300 cursor-not-allowed'}`}
            >
              👤+ 참여하기
            </button>
        ...
        {/* 하단 표준 푸터 (5종 링크) [cite: 2026-02-17] */}
