...
export default function CreateGroup() {
  ...
  const isFormValid = isIljuVisible;
            ...
            <AnimatePresence>
              {isIljuVisible && (
                <motion.div 
                  ...
                  className="bg-slate-50 rounded-[32px] p-8 border border-slate-100 text-center space-y-4 relative overflow-hidden shadow-inner"
                >
          ...
          <section className="pt-10 space-y-6">
            ...
            <div className="space-y-3">
              {[
                ...
                { 
                  q: "띠랑 뭐가 달라요?", 
                  a: "띠는 태어난 해(년)를 기준으로 하지만, 일주는 태어난 날을 기준으로 합니다. 나 자신의 본질적인 기운을 보기에 더 적합합니다." 
                },
                { 
                  q: "왜 일주로 궁합을 봐요?", 
                  a: "사주명리학에서 일주는 '나'를 대표하는 기질이에요. 두 사람의 일주를 비교하면 서로의 기운이 어떻게 만나 어떤 영향을 주는지 알 수 있습니다." 
                },
                { 
                  q: "우리 사이에서 알 수 있는 것", 
                  a: "우리 사이에서는 모임 멤버들의 일주를 분석해서 재미있는 인사이트를 제공해요. 나의 일주 특징, 1:1 궁합 등급, 관계의 시각적 네트워크 등을 한눈에 확인해보세요!" 
                }
              ].map((item, idx) => (
                <div key={idx} className="bg-white rounded-3xl border border-slate-100 overflow-hidden shadow-sm">
                  ...
                  {openAccordion === idx && (
                    <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} className="px-6 pb-6 text-[13px] text-slate-500 leading-7 border-t border-slate-50 pt-4">
                      {item.a}
                    </motion.div>
                  )}
                </div>
